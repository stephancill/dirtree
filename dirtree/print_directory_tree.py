...
def print_directory_tree(tree, indent=""):
  for i, item in enumerate(tree):
    ...
    if isinstance(item, dict):
        for key in item:
            print(f"{indent}├── {key}/") if not is_last_item else print(f"{indent}└── {key}/")
            print_directory_tree(item[key], indent + "    ")
    else:
        print(f"{indent}├── {item}") if not is_last_item else print(f"{indent}└── {item}")
...
def main():
    ...
    parser.add_argument("file_extensions", nargs="+", help="file extensions to search for")
